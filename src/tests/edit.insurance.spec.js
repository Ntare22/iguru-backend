...
describe('EDIT COMPANY', () => {
  ...
  it('should not allow editing insurance company without providing data', (done) => {
    chai
      .request(app)
      .patch(`${prefix}/edit?id=${id}`)
  ...
  it('should should edit insurance company which is not found', (done) => {
    chai
      .request(app)
      .patch(`${prefix}/edit?id=${wrongId}`)
      .set('x-access-token', `${mockToken}`)
      .send(company)
      .end((err, res) => {
  ...
  it('should should edit insurance company', (done) => {
    chai
      .request(app)
      .patch(`${prefix}/edit?id=${id}`)
      .set('x-access-token', `${mockToken}`)
      ...
      .end((err, res) => {
        ...
        expect(res.body).to.have.property(
          'message',
          'Insurance company was updated successfully'
        );
  ...
  it('should have status 500', async () => {
    const results = await companiesController.edit(
      nodeCompany.request,
