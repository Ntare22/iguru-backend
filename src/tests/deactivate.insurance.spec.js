...
describe('COMPANY', () => {
  ...
  it('should should deactivate insurance company which is not found', (done) => {
    chai
      .request(app)
      .patch(`${prefix}/deactivate?id=${wrongId}`)
  ...
  it('should should deactivate insurance company', (done) => {
    chai
      .request(app)
      .patch(`${prefix}/deactivate?id=${id}`)
      .set('x-access-token', `${mockToken}`)
      .end((err, res) => {
        ...
        expect(res.body).to.have.property(
          'message',
          'Insurance company has been deactivated'
        );
  ...
  it('should have status 500', async () => {
    const results = await companiesController.deactivate(
      nodeCompany.request,
